<template>
  <div class="column">
    ...
    <transition enter-active-class="animated bounceInLeft delay-1s slower">
      <essential-employee-list @go-back="backFromList" :company-id="selectedCompanyId" v-if="!showCompanySelect"/>
    </transition>
  </div>
</template>
...
import EssentialEmployeeList from 'components/government/essentialEmployeeList.vue'
...
function getEmployeeAccess () {
  ...
  api.get('/api/government/currentAccess?area=1')
    .then((response) => {
...
function createAccessRequest (company: ICompany | undefined, hasResolution: boolean, hasNote?: string | null) {
  ...
  api.patch('/api/government/createAccessRequest', {
    area: GovernmentArea.EmployeeList,
