<template>
  <div class="column">
    ...
    <transition enter-active-class="animated bounceInLeft delay-1s slower">
      <essential-balance-view-component @go-back="backFromList" :company-id="selectedCompanyId"
...
import EssentialBalanceViewComponent from 'components/government/essentialBalanceViewComponent.vue'
...
function getEmployeeAccess () {
  ...
  api.get('/api/government/currentAccess?area=2')
    .then((response) => {
...
function createAccessRequest (company: ICompany | undefined, hasResolution: boolean, hasNote?: string | null) {
  ...
  api.patch('/api/government/createAccessRequest', {
    area: GovernmentArea.BalanceView,
