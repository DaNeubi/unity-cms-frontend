<template>
  <div class="column">
    ...
    <transition enter-active-class="animated bounceInLeft delay-1s slower">
      <essential-transaction-list-component @go-back="backFromList" :company-id="selectedCompanyId"
                                            v-if="!showCompanySelect" :company-name="selectedCompanyName"/>
    </transition>
  </div>
</template>
...
import EssentialTransactionListComponent from 'components/government/essentialTransactionListComponent.vue'
...
function getEmployeeAccess () {
  ...
  api.get('/api/government/currentAccess?area=3')
    .then((response) => {
...
function createAccessRequest (company: ICompany | undefined, hasResolution: boolean, hasNote?: string | null) {
  ...
  api.patch('/api/government/createAccessRequest', {
    area: GovernmentArea.TransactionList,
