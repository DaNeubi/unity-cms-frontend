<template>
  <div class="column">
    ...
    <transition enter-active-class="animated bounceInLeft delay-1s slower">
      <essential-company-payment-component @go-back="backFromList" :company-id="selectedCompanyId"
...
import EssentialCompanyPaymentComponent from 'components/government/essentialCompanyPaymentComponent.vue'
...
function getEmployeeAccess () {
  ...
  api.get('/api/government/currentAccess?area=4')
    .then((response) => {
...
function createAccessRequest (company: ICompany | undefined, hasResolution: boolean, hasNote?: string | null) {
  ...
  api.patch('/api/government/createAccessRequest', {
    area: GovernmentArea.PaymentList,
